...
import Loading from '../../../components/Loader' // Import your loader
...
export default function TutorialPage() {
  ...
  const handleScrollToTopic = (topicId) => {
    const topicIndex = topicsList.findIndex((t) => t._id === topicId)
  ...
  return (
    <div className="flex flex-col md:flex-row">
      ...
      {/* Sidebar */}
      <aside
        className={`w-full md:w-1/5 p-4 min-h-screen md:block ${
          ...
          {topicsList.map((topic) => (
            <li key={topic._id}>
              <button
                ...
                className="w-full text-left text-gray-700 dark:text-gray-300 font-medium px-2 py-1 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-md transition"
              >
                {topic.heading}
              </button>
            </li>
      ...
      <div className="w-full md:w-4/5 p-6 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-200">
        <div className="border-b-2 mb-6 border-gray-500">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">Author: {tutorial?.author}</p>
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Last updated:{' '}
            {tutorial?.updatedDate
              ? new Date(tutorial.updatedDate).toLocaleDateString()
              : 'N/A'}
          </p>
        </div>
        ...
        <div className="space-y-6">
          {paginatedTopics.map((topic) => (
            <div key={topic._id} id={topic._id}>
              <h3 className="text-2xl font-semibold text-gray-800 dark:text-gray-300 mb-2">
                {topic.heading}
              </h3>
        ...
        {totalPages > 1 && (
          <div className="flex justify-center mt-6">
            <ReactPaginate
              ...
              pageLinkClassName={
                'px-4 py-2 bg-gray-700 text-white hover:bg-green-400 cursor-pointer block rounded-md'
              }
              activeClassName={'bg-green-500 text-white rounded-md'}
              activeLinkClassName={'bg-green-500 text-white block px-4 py-2 rounded-md'}
              previousClassName={
                'px-4 py-2 bg-gray-700 text-white rounded-md hover:bg-gray-600 cursor-pointer'
              }
              nextClassName={
                'px-4 py-2 bg-gray-700 text-white rounded-md hover:bg-gray-600 cursor-pointer'
              }
